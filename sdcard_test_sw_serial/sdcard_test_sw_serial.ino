...
{ 
  ...
  Serial.begin(9600);
  mySerial.begin(9600); //Much faster serial, used for testing buffer overruns on OpenLog
  ...
  for(int numofTests = 0 ; numofTests < testAmt ; numofTests++)
  {
    ...
    for(int k = 33; k < 43 ; k++)
    {
      ...
      mySerial.write(k); //Print the ASCII value directly: ! then " then #, etc
      mySerial.println("hihihi");
      //delay(50);
